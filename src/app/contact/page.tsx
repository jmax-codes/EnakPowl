  ...
  return (
    <SmoothScroll>
      ...
      <main ref={containerRef} className="bg-brand-red min-h-[300vh] text-white pt-[30vh] pb-20 px-6 md:px-12 flex flex-col items-center">
        ...
        <section className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-24 mb-48 items-center bg-brand-red relative z-10">
          <motion.div style={{ x: sisterX }}>
            <div className="relative aspect-[4/5] bg-black/20 rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl group">
               ...
               <div className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-black/80 to-transparent">
                 <h3 className="text-3xl font-black text-brand-yellow uppercase tracking-tighter">Two Sisters, One Dream</h3>
                 <p className="text-white/70 mt-2 font-medium">Stefanie Agracia & Selvi Oktavianie</p>
               </div>
            </div>
          </motion.div>

          <motion.div style={{ x: infoX }} className="flex flex-col gap-10">
        ...
        <motion.div 
          style={{ scale: useTransform(scrollYProgress, [0.8, 1], [0.9, 1]) }}
          className="w-full max-w-4xl text-center p-16 bg-brand-yellow rounded-[4rem] border-[6px] border-white shadow-[0_40px_100px_rgba(0,0,0,0.5)] mb-32 group relative overflow-hidden"
        >
          <div className="absolute top-0 left-0 w-full h-[15px]" style={{ backgroundImage: "linear-gradient(to right, #C41E3A 50%, transparent 50%)", backgroundSize: "30px 100%" }} />
          
          <h2 className="text-4xl md:text-5xl font-black text-brand-red uppercase tracking-tighter mb-6 leading-tight">
          ...
          <motion.a 
            ...
            className="text-3xl md:text-5xl font-black text-white block tracking-tight drop-shadow-md hover:text-brand-red transition-colors"
          >
            +62 87868898855
          </motion.a>

          <div className="absolute bottom-0 left-0 w-full h-[15px]" style={{ backgroundImage: "linear-gradient(to right, #C41E3A 50%, transparent 50%)", backgroundSize: "30px 100%" }} />
        </motion.div>
